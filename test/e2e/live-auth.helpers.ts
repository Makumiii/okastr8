...
export function createLiveTestToken(userId = 'e2e-live-probe'): string {
  ...
  if (!tokenLine) {
    throw new Error(`Token output missing TOKEN_JSON marker. Output: ${result.stdout} ${result.stderr}`);
  }
...
export async function setSessionCookie(context: import('@playwright/test').BrowserContext, token: string, baseUrl: string): Promise<void> {
