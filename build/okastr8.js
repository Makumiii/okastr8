...
__export(exports_systemd, {
  ...
  addSystemdCommands: () => addSystemdCommands
});
...
var SCRIPT_BASE_PATH;
var init_systemd = __esm(() => {
  init_command();
  SCRIPT_BASE_PATH = path.join(process.cwd(), "scripts", "systemd");
});
...
async function deployWithDocker(options, config) {
  ...
  const envFilePath = join10(releasePath, ".env");
  ...
  const { stopService: stopService2, disableService: disableService2 } = await Promise.resolve().then(() => (init_systemd(), exports_systemd));
  const serviceNames = [appName, appName.replace(/-/g, "_")];
  for (const serviceName of serviceNames) {
    try {
      log(`Checking legacy systemd service: ${serviceName}...`);
      await stopService2(serviceName).catch(() => {});
      await disableService2(serviceName).catch(() => {});
    } catch {}
  }
...
import { readFile as readFile7, mkdir as mkdir5, rm as rm2 } from "fs/promises";
...
async function importRepo(options, deploymentId) {
  const { createApp } = await Promise.resolve().then(() => (init_app(), exports_app));
...
Visit https://github.com/${repo.full_name}/new/${branch} to create the file.`,
    ...
    const cleanupFailedDeployment = async (reason) => {
      ...
      try {
        ...
        const composePath = join14(releasePath, "docker-compose.yml");
        if (existsSync9(composePath)) {
...
var __filename4, __dirname4, PROJECT_ROOT3, APPS_DIR5, GITHUB_API = "https://api.github.com";
var init_github = __esm(() => {
  ...
  PROJECT_ROOT3 = join14(__dirname4, "..", "..");
  APPS_DIR5 = join14(OKASTR8_HOME, "apps");
});
...
__export(exports_app, {
  ...
  stopApp: () => stopApp,
  startApp: () => startApp,
  setAppWebhookAutoDeploy: () => setAppWebhookAutoDeploy,
  restartApp: () => restartApp,
  listApps: () => listApps,
  getAppStatus: () => getAppStatus,
  getAppMetadata: () => getAppMetadata,
  getAppLogs: () => getAppLogs,
  exportAppLogs: () => exportAppLogs,
  deleteApp: () => deleteApp,
  createApp: () => createApp,
  addAppCommands: () => addAppCommands
});
import { join as join15, dirname as dirname5 } from "path";
import { mkdir as mkdir6, writeFile as writeFile8, readFile as readFile8, rm as rm3, readdir as readdir2 } from "fs/promises";
...
async function createApp(config) {
  try {
    ...
    await writeFile8(metadataPath, JSON.stringify(metadata, null, 2));
    return {
      success: true,
      appDir,
...
async function deleteApp(appName) {
  try {
    ...
    await stopContainer(appName).catch(() => {});
    await removeContainer(appName).catch(() => {});
    const currentComposePath = join15(APPS_DIR6, appName, "current", "docker-compose.yml");
    if (existsSync10(currentComposePath)) {
    ...
    const appDir = join15(APPS_DIR6, appName);
    await rm3(appDir, { recursive: true, force: true }).catch(() => {});
    return {
      success: false,
...
async function listApps() {
  try {
    await mkdir6(APPS_DIR6, { recursive: true });
    const entries = await readdir2(APPS_DIR6, { withFileTypes: true });
    const apps = [];
    const runningContainers = await listContainers();
    const runningMap = new Map(runningContainers.map((c) => [c.name, c.state]));
    for (const entry of entries) {
      if (entry.isDirectory()) {
        const metadataPath = join15(APPS_DIR6, entry.name, "app.json");
        try {
          const metadata = JSON.parse(await readFile8(metadataPath, "utf-8"));
...
async function getAppMetadata(appName) {
  ...
  const metadataPath = join15(appDir, "app.json");
  try {
    const content = await readFile8(metadataPath, "utf-8");
    return JSON.parse(content);
  } catch {
...
var __filename5, __dirname5, PROJECT_ROOT4, APPS_DIR6;
var init_app = __esm(() => {
  init_command();
  init_deploy_core();
  init_version();
...
async function getBrevoConfig() {
  try {
    if (!existsSync11(SYSTEM_YAML_PATH)) {
      console.error("Email: system.yaml not found");
      return null;
    }
    const content = await readFile9(SYSTEM_YAML_PATH, "utf-8");
...
var init_email = __esm(() => {
  ...
  SYSTEM_YAML_PATH = join16(homedir4(), ".okastr8", "system.yaml");
});
...
import { readFile as readFile10, writeFile as writeFile9, mkdir as mkdir7 } from "fs/promises";
var __filename6 = fileURLToPath5(import.meta.url);
var __dirname6 = dirname6(__filename6);
var PROJECT_ROOT5 = join17(__dirname6, "..", "..");
var APPS_DIR7 = join17(OKASTR8_HOME, "apps");
var DEPLOYMENT_FILE = join17(OKASTR8_HOME, "deployment.json");
var SCRIPTS2 = {
  gitPull: join17(PROJECT_ROOT5, "scripts", "git", "pull.sh"),
  gitRollback: join17(PROJECT_ROOT5, "scripts", "git", "rollback.sh"),
  healthCheck: join17(PROJECT_ROOT5, "scripts", "deploy", "health-check.sh"),
  restart: join17(PROJECT_ROOT5, "scripts", "systemd", "restart.sh"),
  stop: join17(PROJECT_ROOT5, "scripts", "systemd", "stop.sh"),
  start: join17(PROJECT_ROOT5, "scripts", "systemd", "start.sh")
};
...
async function deployApp(options) {
  ...
  try {
    const { getAppMetadata: getAppMetadata2, updateApp: updateApp2 } = await Promise.resolve().then(() => (init_app(), exports_app));
    if (branch) {
      try {
...
async function rollbackApp(appName, commitHash) {
  ...
  try {
    if (!commitHash) {
      const content = await readFile10(DEPLOYMENT_FILE, "utf-8");
...
var SSH_KEY_PATH = join18(homedir5(), ".ssh", "okastr8_deploy_key");
...
`);
      ...
      const { listApps: listApps2 } = await Promise.resolve().then(() => (init_app(), exports_app));
      const { apps: deployedApps } = await listApps2();
      ...
      const Enquirer = (await Promise.resolve().then(() => __toESM(require_enquirer(), 1))).default;
...
\u274C ${result.message}`);
  ...
  github.command("setup-key").description("Setup SSH deploy key for passwordless cloning").action(async () => {
    try {
      ...
      if (!existsSync12(pubKeyPath)) {
        console.log("Generating new SSH deploy key...");
        const sshDir = join18(homedir5(), ".ssh");
      ...
      const publicKey = (await readFile11(pubKeyPath, "utf-8")).trim();
...
`)) {
      ...
      if (memory === 0) {
        ...
        if (existsSync13(cgroupPath)) {
          try {
            const memContent = await readFile12(cgroupPath, "utf-8");
...
var SCRIPTS3 = {
  install: join19(PROJECT_ROOT6, "scripts", "tunnel", "install.sh")
};
...
async function controlAllServices(action) {
  ...
  for (const app of apps) {
    ...
    try {
      if (action === "start")
        await startService(app.name);
      if (action === "stop")
        await stopService(app.name);
      if (action === "restart")
        await restartService(app.name);
