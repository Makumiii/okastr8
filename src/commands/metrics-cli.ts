...
function renderSystemOverview(system: SystemMetrics): string {
    ...
    lines.push(`CPU: ${cpuColor}${cpuBar}${RESET} ${padLeft(system.cpu.usage.toFixed(1), 5)}%  ${system.cpu.cores} cores @ ${system.cpu.speed}MHz  Steal ${formatPercent(system.cpu.steal)}`);
    ...
    lines.push(`MEM: ${memColor}${memBar}${RESET} ${padLeft(system.memory.percent.toFixed(1), 5)}%  ${formatGbFromMb(system.memory.used)} / ${formatGbFromMb(system.memory.total)}  Free ${formatGbFromMb(system.memory.free)}  Avail ${formatGbFromMb(system.memory.available)}`);
    ...
    lines.push(`SWP: ${swapColor}${swapBar}${RESET} ${padLeft(system.swap.percent.toFixed(1), 5)}%  ${formatGbFromMb(system.swap.used)} / ${formatGbFromMb(system.swap.total)}  In ${system.swap.inRate.toFixed(1)} MB/m  Out ${system.swap.outRate.toFixed(1)} MB/m`);

    const loadLine = `${system.load.avg1.toFixed(2)} ${system.load.avg5.toFixed(2)} ${system.load.avg15.toFixed(2)}`;
    const bootLine = system.health.bootTime ? new Date(system.health.bootTime * 1000).toLocaleString() : '-';
    lines.push(`Load: ${loadLine}  Uptime: ${system.uptime.readable}  Boot: ${bootLine}`);

    lines.push(`Disk I/O: R ${formatRate(system.disk.io.readPerSec)}  W ${formatRate(system.disk.io.writePerSec)}  Busy ${formatPercent(system.disk.io.busyPercent)}  Lat ${system.disk.io.latencyMs.toFixed(1)}ms`);
    const blocks: string[] = [];
    ...
    if (system.disk.mounts.length === 0) {
        ...
        diskLines.push(`${DIM}${padRight('MOUNT', 18)} ${padLeft('USED', 10)} ${padLeft('TOTAL', 10)} ${padLeft('%', 6)} ${padLeft('FREE', 10)} ${padLeft('INODES%', 8)}${RESET}`);
        for (const mount of system.disk.mounts) {
            diskLines.push(`${padRight(mount.mount, 18)} ${padLeft(formatBytes(mount.used), 10)} ${padLeft(formatBytes(mount.total), 10)} ${padLeft(formatPercent(mount.percent, 0), 6)} ${padLeft(formatBytes(mount.free), 10)} ${padLeft(formatPercent(mount.inodesPercent, 0), 8)}`);
        }
    }
    blocks.push(makeBox('Disk Mounts', diskLines, width));

    const networkLines: string[] = [];
    networkLines.push(`Totals: RX ${formatRate(system.network.totalRxPerSec)}  TX ${formatRate(system.network.totalTxPerSec)}  Active ${system.network.activeConnections}  Retransmits ${Number.isFinite(system.network.retransmits) ? system.network.retransmits : '-'}`);
    if (system.network.interfaces.length === 0) {
        networkLines.push(`${DIM}No interface data available.${RESET}`);
    } else {
        networkLines.push(`${DIM}${padRight('IFACE', 10)} ${padLeft('RX', 10)} ${padLeft('TX', 10)} ${padLeft('RXERR', 6)} ${padLeft('TXERR', 6)} ${padLeft('RXDROP', 7)} ${padLeft('TXDROP', 7)}${RESET}`);
        for (const iface of system.network.interfaces) {
            networkLines.push(`${padRight(iface.name, 10)} ${padLeft(formatBytes(iface.rxBytes), 10)} ${padLeft(formatBytes(iface.txBytes), 10)} ${padLeft(String(iface.rxErrors), 6)} ${padLeft(String(iface.txErrors), 6)} ${padLeft(String(iface.rxDropped), 7)} ${padLeft(String(iface.txDropped), 7)}`);
        }
    }
    blocks.push(makeBox('Network', networkLines, width));

    const healthLines: string[] = [];
    healthLines.push(`OOM Kills: ${system.health.oomKills}  Unexpected Reboot: ${system.health.unexpectedReboot ? 'yes' : 'no'}`);
    healthLines.push(`File Descriptors: ${system.limits.fileDescriptors.used}/${system.limits.fileDescriptors.total} (${formatPercent(system.limits.fileDescriptors.percent, 0)})`);
...
function renderAppSummary(apps: AppMetrics[]): string {
    ...
    lines.push(`${DIM}${padRight('NAME', 22)} ${padLeft('STATUS', 8)} ${padLeft('CPU%', 6)} ${padLeft('MEM%', 6)} ${padRight('UPTIME', 10)} ${padLeft('REQ/S', 7)} ${padLeft('4XX', 5)} ${padLeft('5XX', 5)}${RESET}`);

    for (const app of apps) {
        const statusColor = getStatusColor(app.status);
        const name = padRight(app.name, 22);
        const status = padLeft(app.status, 8);
        const cpu = app.status === 'running' ? padLeft(app.resources.cpu.toFixed(1), 6) : padLeft('-', 6);
        const mem = app.status === 'running' ? padLeft(app.resources.memoryPercent.toFixed(1), 6) : padLeft('-', 6);
        const uptime = app.status === 'running' ? padRight(app.uptime.readable, 10) : padRight('-', 10);
        const reqs = app.status === 'running'
            ? padLeft(app.traffic.requestsPerSec.toFixed(1), 7)
            : padLeft('-', 7);
        const err4xx = app.status === 'running' ? padLeft(app.traffic.errorRate4xx.toFixed(1), 5) : padLeft('-', 5);
        const err5xx = app.status === 'running' ? padLeft(app.traffic.errorRate5xx.toFixed(1), 5) : padLeft('-', 5);

        lines.push(`${name} ${statusColor}${status}${RESET} ${cpu} ${mem} ${uptime} ${reqs} ${err4xx} ${err5xx}`);
    }
...
function renderAppDetail(app: AppMetrics): string {
    ...
    lines.push(`CPU: ${formatPercent(app.resources.cpu)}  Memory: ${app.resources.memory} MB (${formatPercent(app.resources.memoryPercent)})  Limit: ${app.resources.memoryLimit} MB`);
    lines.push(`Throttling: ${app.resources.throttling !== undefined ? formatPercent(app.resources.throttling) : '-'}`);
    lines.push(`Health: ${app.health.status}  Failing streak: ${app.health.failingStreak}`);
    lines.push(`Restarts: ${app.restarts.count}  Last exit: ${app.restarts.lastExitCode ?? '-'}  Reason: ${app.restarts.lastExitReason || '-'}`);
    lines.push(`Disk: ${formatBytes(app.disk.used)} used`);
    lines.push(`Traffic: Total ${app.traffic.totalRequests}  RPS ${app.traffic.requestsPerSec.toFixed(2)}  4xx ${formatPercent(app.traffic.errorRate4xx)}  5xx ${formatPercent(app.traffic.errorRate5xx)}  P95 ${app.traffic.p95Latency !== undefined ? `${Math.round(app.traffic.p95Latency)}ms` : '-'}`);
    const width = getTerminalWidth();
...
async function selectAppView(apps: AppMetrics[], modeArg?: string, appArg?: string): Promise<AppViewSelection> {
...
async function resolveViewConfig(group?: string, appMode?: string, appName?: string): Promise<ViewConfig> {
        ...
        if (view.group === 'apps' || view.group === 'all') {
            if (metrics.apps.length === 0) {
                ...
                if (app) {
                    ...
                    console.log(`${DIM}App not found: ${view.appView.appName}${RESET}`);
                }
            } else {
