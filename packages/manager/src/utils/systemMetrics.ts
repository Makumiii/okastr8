...
export async function getDetailedSystemMetrics(): Promise<SystemMetrics> {
    ...
    try {
        ...
        const parseVm = (key: string) =>
            parseInt(vmLines.find((l) => l.startsWith(key))?.split(/\s+/)[1] || "0", 10);
        ...
        if (lastState && lastState.swap) {
            ...
            if (timeDeltaMin > 0) {
                swapInRate = Math.max(
                    0,
                    ((pswpin - lastState.swap.in) * 4096) / 1024 / 1024 / timeDeltaMin
                ); // pages to MB/min
                swapOutRate = Math.max(
                    0,
                    ((pswpout - lastState.swap.out) * 4096) / 1024 / 1024 / timeDeltaMin
                );
            }
        }
    ...
    try {
        ...
        for (const line of inodeLines) {
            ...
            if (parts.length >= 6)
                inodeMap[parts[5]!] = parseInt((parts[4] || "0").replace("%", ""), 10);
        }
        ...
        for (const line of spaceLines) {
            ...
            if (parts.length >= 7) {
                ...
                if (
                    !fsType ||
                    !mount ||
                    EXCLUDED_FS_TYPES.includes(fsType) ||
                    EXCLUDED_PATH_PREFIXES.some((p) => mount.startsWith(p))
                )
                    continue;
    ...
    try {
        ...
        if (lastState && lastState.disk) {
            ...
            if (timeDeltaMs > 0) {
                diskReadRate = Math.max(
                    0,
                    (totalRead - (lastState.disk["_total"]?.read || 0)) / (timeDeltaMs / 1000)
                );
                diskWriteRate = Math.max(
                    0,
                    (totalWrite - (lastState.disk["_total"]?.write || 0)) / (timeDeltaMs / 1000)
                );
                diskBusyPercent = Math.min(
                    100,
                    ((totalActiveTime - (lastState.disk["_total"]?.active || 0)) / timeDeltaMs) *
                        100
                );
            }
        }
        if (lastState)
            lastState.disk["_total"] = {
                read: totalRead,
                write: totalWrite,
    ...
    return {
        ...
        limits: {
            fileDescriptors: {
                used: fdUsed,
                total: fdTotal,
                percent: parseFloat(fdPercent.toFixed(1)),
            },
        },
