...
for DOCKER_CMD in "${DOCKER_BINARIES[@]}"; do
  ...
  SUDOERS_CONTENT+="$TARGET_USER ALL=(root) NOPASSWD: $DOCKER_CMD build *\n"
  SUDOERS_CONTENT+="$TARGET_USER ALL=(root) NOPASSWD: $DOCKER_CMD buildx version\n"
  SUDOERS_CONTENT+="$TARGET_USER ALL=(root) NOPASSWD: $DOCKER_CMD buildx build *\n"
